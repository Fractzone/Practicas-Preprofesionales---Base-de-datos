...
from modelo.coordinadores import TutorAcademico, RepositorioTutorAcademico
...
from vista.vista_crud_personal.interfaz_vista_crud_personal import VistaCrudTutorAcademico
from controlador.sincronizador_credenciales import SincronizadorCredenciales

class ControladorCRUDTutorAcademico:

    def __init__(self, persistencia):
        self.persistencia = persistencia
        self.repo = RepositorioTutorAcademico(persistencia)
        self.vista = VistaCrudTutorAcademico()
    ...
    def slot_eliminar(self):
        ...
        try:
            tutor = self.repo.buscar(cedula)
            if tutor is None:
                raise ValueError("No existe un tutor académico con la cédula ingresada.")
